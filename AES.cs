using System;
using System.Collections.Generic;

namespace Cryptooo {
    public class AES {
        public byte[] Decode(byte[] message, byte[] keyByte) {
            List<byte[]> paddedMessage = SplittingMessage(message);
            //byte[] keyByte = KeyToBytes(key);
            byte[] result = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE * paddedMessage.Count];
            int k = 0;

            for (int i = 0; i < paddedMessage.Count; i++) {
                AES_Decrypt(paddedMessage[i], keyByte);
                for (int j = 0; j < NUMBER_OF_ELEMENTS_IN_THE_STATE; j++) {
                    result[k] = paddedMessage[i][j];
                    k++;
                }
            }

            return result;
        }

        public byte[] Encode(byte[] message, byte[] keyByte) {
            List<byte[]> paddedMessage = SplittingMessage(message);
            //byte[] keyByte = KeyToBytes(key);
            byte[] result = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE * paddedMessage.Count];
            int k = 0;

            for (int i = 0; i < paddedMessage.Count; i++) {
                AES_Encrypt(paddedMessage[i], keyByte);
                for (int j = 0; j < NUMBER_OF_ELEMENTS_IN_THE_STATE; j++) {
                    result[k] = paddedMessage[i][j];
                    k++;
                }
            }

            return result;
        }

        private const int TOTAL_NUMBER_OF_BYTES = 176;
        private const int NUMBER_OF_ELEMENTS_IN_THE_STATE = 16;
        private const int NUMBER_OF_ROUNDS = 9;
        private const int NUMBER_OF_ELEMENTS_IN_THE_COLUMN = 4;

        private readonly byte[] sBox = new byte[] {
             };
        private readonly byte[] sBoxInv = new byte[] {
             };
        private readonly byte[] mul2 = new byte[] {
             };
        private readonly byte[] mul3 = new byte[] {
             };
        private readonly byte[] mul9 = new byte[] {
             };
        private readonly byte[] mul11 = new byte[] {
             };
        private readonly byte[] mul13 = new byte[] {
             };
        private readonly byte[] mul14 = new byte[] {
             };
        private readonly byte[] rCon = new byte[] {
             };

        // сдвигает байты на один влево, каждый байт меняет на соответствующий ему из sBox
        // в зависимости от итерации меняет первый байт на соответствующий из rCon
        private void KeyExpansionCore(byte[] byteIn, byte i) {
            byte t = byteIn[0];
            byteIn[0] = byteIn[1];
            byteIn[1] = byteIn[2];
            byteIn[2] = byteIn[3];
            byteIn[3] = t;

            byteIn[0] = sBox[byteIn[0]];
            byteIn[1] = sBox[byteIn[1]];
            byteIn[2] = sBox[byteIn[2]];
            byteIn[3] = sBox[byteIn[3]];

            byteIn[0] ^= rCon[i];
        }

        // генерирует ключ из 176 и заполняет (на основе существующего ключа)
        private void KeyExpansion(byte[] inputKey, byte[] expandedKeys) {
            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                expandedKeys[i] = inputKey[i];
            }

            byte bytesGenerated = NUMBER_OF_ELEMENTS_IN_THE_STATE;
            byte rConIteration = 1;
            byte[] tmp = new byte[NUMBER_OF_ELEMENTS_IN_THE_COLUMN];

            while (bytesGenerated < TOTAL_NUMBER_OF_BYTES) {
                for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_COLUMN; i++) {
                    tmp[i] = expandedKeys[i + bytesGenerated - NUMBER_OF_ELEMENTS_IN_THE_COLUMN];
                }

                if (bytesGenerated % NUMBER_OF_ELEMENTS_IN_THE_STATE == 0) {
                    KeyExpansionCore(tmp, rConIteration);
                    rConIteration++;
                }

                for (byte a = 0; a < NUMBER_OF_ELEMENTS_IN_THE_COLUMN; a++) {
                    expandedKeys[bytesGenerated] = (byte)(expandedKeys[bytesGenerated - NUMBER_OF_ELEMENTS_IN_THE_STATE] ^ tmp[a]);
                    bytesGenerated++;
                }
            }
        }

        // меняет каждый байт на соотвествующий ему из таблицы sBox
        private void SubBytes(byte[] state) {
            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = sBox[state[i]];
            }
        }

        private void InvSubBytes(byte[] state) {
            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = sBoxInv[state[i]];
            }
        }

        // сдвигает матрицу 4х4: первая строка такая же, вторая сдвигается на 1 байт влево
        // третья - на два байта влево, четвертая - на три        
        private void ShiftRows(byte[] state) {
            byte[] tmp = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
            tmp[0] = state[0];
            tmp[1] = state[5];
            tmp[2] = state[10];
            tmp[3] = state[15];

            tmp[4] = state[4];
            tmp[5] = state[9];
            tmp[6] = state[14];
            tmp[7] = state[3];

            tmp[8] = state[8];
            tmp[9] = state[13];
            tmp[10] = state[2];
            tmp[11] = state[7];

            tmp[12] = state[12];
            tmp[13] = state[1];
            tmp[14] = state[6];
            tmp[15] = state[11];

            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = tmp[i];
            }
        }

        private void InvShiftRows(byte[] state) {
            byte[] tmp = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
            tmp[0] = state[0];
            tmp[1] = state[13];
            tmp[2] = state[10];
            tmp[3] = state[7];

            tmp[4] = state[4];
            tmp[5] = state[1];
            tmp[6] = state[14];
            tmp[7] = state[11];

            tmp[8] = state[8];
            tmp[9] = state[5];
            tmp[10] = state[2];
            tmp[11] = state[15];

            tmp[12] = state[12];
            tmp[13] = state[9];
            tmp[14] = state[6];
            tmp[15] = state[3];

            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = tmp[i];
            }
        }

        // меняет каждый байт умножением матриц (используются mul)
        private void MixColumns(byte[] state) {
            byte[] tmp = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];

            tmp[0] = (byte)(mul2[state[0]] ^ mul3[state[1]] ^ state[2] ^ state[3]);
            tmp[1] = (byte)(state[0] ^ mul2[state[1]] ^ mul3[state[2]] ^ state[3]);
            tmp[2] = (byte)(state[0] ^ state[1] ^ mul2[state[2]] ^ mul3[state[3]]);
            tmp[3] = (byte)(mul3[state[0]] ^ state[1] ^ state[2] ^ mul2[state[3]]);

            tmp[4] = (byte)(mul2[state[4]] ^ mul3[state[5]] ^ state[6] ^ state[7]);
            tmp[5] = (byte)(state[4] ^ mul2[state[5]] ^ mul3[state[6]] ^ state[7]);
            tmp[6] = (byte)(state[4] ^ state[5] ^ mul2[state[6]] ^ mul3[state[7]]);
            tmp[7] = (byte)(mul3[state[4]] ^ state[5] ^ state[6] ^ mul2[state[7]]);

            tmp[8] = (byte)(mul2[state[8]] ^ mul3[state[9]] ^ state[10] ^ state[11]);
            tmp[9] = (byte)(state[8] ^ mul2[state[9]] ^ mul3[state[10]] ^ state[11]);
            tmp[10] = (byte)(state[8] ^ state[9] ^ mul2[state[10]] ^ mul3[state[11]]);
            tmp[11] = (byte)(mul3[state[8]] ^ state[9] ^ state[10] ^ mul2[state[11]]);

            tmp[12] = (byte)(mul2[state[12]] ^ mul3[state[13]] ^ state[14] ^ state[15]);
            tmp[13] = (byte)(state[12] ^ mul2[state[13]] ^ mul3[state[14]] ^ state[15]);
            tmp[14] = (byte)(state[12] ^ state[13] ^ mul2[state[14]] ^ mul3[state[15]]);
            tmp[15] = (byte)(mul3[state[12]] ^ state[13] ^ state[14] ^ mul2[state[15]]);

            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = tmp[i];
            }
        }

        private void InvMixColumns(byte[] state) {
            byte[] tmp = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];

            tmp[0] = (byte)(mul14[state[0]] ^ mul11[state[1]] ^ mul13[state[2]] ^ mul9[state[3]]);
            tmp[1] = (byte)(mul9[state[0]] ^ mul14[state[1]] ^ mul11[state[2]] ^ mul13[state[3]]);
            tmp[2] = (byte)(mul13[state[0]] ^ mul9[state[1]] ^ mul14[state[2]] ^ mul11[state[3]]);
            tmp[3] = (byte)(mul11[state[0]] ^ mul13[state[1]] ^ mul9[state[2]] ^ mul14[state[3]]);

            tmp[4] = (byte)(mul14[state[4]] ^ mul11[state[5]] ^ mul13[state[6]] ^ mul9[state[7]]);
            tmp[5] = (byte)(mul9[state[4]] ^ mul14[state[5]] ^ mul11[state[6]] ^ mul13[state[7]]);
            tmp[6] = (byte)(mul13[state[4]] ^ mul9[state[5]] ^ mul14[state[6]] ^ mul11[state[7]]);
            tmp[7] = (byte)(mul11[state[4]] ^ mul13[state[5]] ^ mul9[state[6]] ^ mul14[state[7]]);

            tmp[8] = (byte)(mul14[state[8]] ^ mul11[state[9]] ^ mul13[state[10]] ^ mul9[state[11]]);
            tmp[9] = (byte)(mul9[state[8]] ^ mul14[state[9]] ^ mul11[state[10]] ^ mul13[state[11]]);
            tmp[10] = (byte)(mul13[state[8]] ^ mul9[state[9]] ^ mul14[state[10]] ^ mul11[state[11]]);
            tmp[11] = (byte)(mul11[state[8]] ^ mul13[state[9]] ^ mul9[state[10]] ^ mul14[state[11]]);

            tmp[12] = (byte)(mul14[state[12]] ^ mul11[state[13]] ^ mul13[state[14]] ^ mul9[state[15]]);
            tmp[13] = (byte)(mul9[state[12]] ^ mul14[state[13]] ^ mul11[state[14]] ^ mul13[state[15]]);
            tmp[14] = (byte)(mul13[state[12]] ^ mul9[state[13]] ^ mul14[state[14]] ^ mul11[state[15]]);
            tmp[15] = (byte)(mul11[state[12]] ^ mul13[state[13]] ^ mul9[state[14]] ^ mul14[state[15]]);

            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = tmp[i];
            }
        }

        // каждый байт сообщения и ключа складывается по модулю 2
        private void AddRoundKey(byte[] state, byte[] roundKey) {
            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] ^= roundKey[i];
            }
        }

        public void AES_Encrypt(byte[] message, byte[] key) {
            byte[] state = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = message[i];
            }

            byte[] expandedKey = new byte[TOTAL_NUMBER_OF_BYTES];
            byte[] expandedKeyCopy = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
            KeyExpansion(key, expandedKey);
            AddRoundKey(state, key);

            for (int i = 0; i < NUMBER_OF_ROUNDS; i++) {
                SubBytes(state);
                ShiftRows(state);
                MixColumns(state);
                Array.Copy(expandedKey, NUMBER_OF_ELEMENTS_IN_THE_STATE * (i + 1), expandedKeyCopy, 0, NUMBER_OF_ELEMENTS_IN_THE_STATE);
                AddRoundKey(state, expandedKeyCopy);
            }

            SubBytes(state);
            ShiftRows(state);
            Array.Copy(expandedKey, TOTAL_NUMBER_OF_BYTES - NUMBER_OF_ELEMENTS_IN_THE_STATE, expandedKeyCopy, 0, NUMBER_OF_ELEMENTS_IN_THE_STATE);
            AddRoundKey(state, expandedKeyCopy);

            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                message[i] = state[i];
            }
        }

        public void AES_Decrypt(byte[] message, byte[] key) {
            byte[] state = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                state[i] = message[i];
            }

            byte[] expandedKey = new byte[TOTAL_NUMBER_OF_BYTES];
            byte[] expandedKeyCopy = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];

            KeyExpansion(key, expandedKey);
            Array.Copy(expandedKey, TOTAL_NUMBER_OF_BYTES - NUMBER_OF_ELEMENTS_IN_THE_STATE, expandedKeyCopy, 0, NUMBER_OF_ELEMENTS_IN_THE_STATE);
            AddRoundKey(state, expandedKeyCopy);

            for (int i = NUMBER_OF_ROUNDS; i > 0; i--) {
                InvShiftRows(state);
                InvSubBytes(state);

                Array.Copy(expandedKey, NUMBER_OF_ELEMENTS_IN_THE_STATE * i, expandedKeyCopy, 0, NUMBER_OF_ELEMENTS_IN_THE_STATE);
                AddRoundKey(state, expandedKeyCopy);
                InvMixColumns(state);
            }

            InvShiftRows(state);
            InvSubBytes(state);
            AddRoundKey(state, key);

            for (int i = 0; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                message[i] = state[i];
            }
        }

        public List<byte[]> SplittingMessage(byte[] message) {
            int originalLen = message.Length;
            int lenOfPaddedMessage = originalLen;


            if (lenOfPaddedMessage % NUMBER_OF_ELEMENTS_IN_THE_STATE != 0) {
                lenOfPaddedMessage = (lenOfPaddedMessage / NUMBER_OF_ELEMENTS_IN_THE_STATE + 1) * NUMBER_OF_ELEMENTS_IN_THE_STATE;
            }

            byte[] messageCopy = new byte[lenOfPaddedMessage];


            for (int i = 0; i < lenOfPaddedMessage; i++) {
                if (i >= originalLen) {
                    messageCopy[i] = 0;
                } else {
                    messageCopy[i] = message[i];
                }
            }

            List<byte[]> paddedMessage = new List<byte[]>();
            for (int i = 0; i < lenOfPaddedMessage; i += NUMBER_OF_ELEMENTS_IN_THE_STATE) {
                byte[] tmp = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
                Array.Copy(messageCopy, i, tmp, 0, NUMBER_OF_ELEMENTS_IN_THE_STATE);
                paddedMessage.Add(tmp);
            }

            return paddedMessage;
        }

        public byte[] KeyToBytes(string str) {
            byte[] key = new byte[NUMBER_OF_ELEMENTS_IN_THE_STATE];
            for (int i = 0; i < str.Length; i++) {
                key[i] = (byte)str[i];
            }

            for (int i = str.Length; i < NUMBER_OF_ELEMENTS_IN_THE_STATE; i++) {
                key[i] = 0;
            }
            return key;
        }
    }
}
