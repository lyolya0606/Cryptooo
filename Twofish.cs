    ...
    internal class Twofish {
        ...
        */
        ...
        private void SetKey(byte[] key) {
            ...
            for (int i = 0; i < k64Cnt; i++) {
                ...
                k32e[i] = (int)LE_To_UInt32(key, p);
                k32o[i] = (int)LE_To_UInt32(key, p + 4);
            ...
            for (int i = 0; i < TOTAL_SUBKEYS / 2; i++) {
                ...
                gSubKeys[i * 2] = A;
                A += B;
                gSubKeys[i * 2 + 1] = RotateLeft(A, SK_ROTL);
            }
        ...
        */
        ...
        private void EncryptBlock(
            ...
            byte[] dst,
            int dstIndex) {
            int x0 = (int)LE_To_UInt32(src, srcIndex) ^ gSubKeys[INPUT_WHITEN];
            int x1 = (int)LE_To_UInt32(src, srcIndex + 4) ^ gSubKeys[INPUT_WHITEN + 1];
            int x2 = (int)LE_To_UInt32(src, srcIndex + 8) ^ gSubKeys[INPUT_WHITEN + 2];
            int x3 = (int)LE_To_UInt32(src, srcIndex + 12) ^ gSubKeys[INPUT_WHITEN + 3];
            ...
            for (int r = 0; r < ROUNDS; r += 2) {
                ...
                x2 ^= t0 + t1 + gSubKeys[k++];
                x2 = RotateRight(x2, 1);
                x3 = RotateLeft(x3, 1) ^ (t0 + 2 * t1 + gSubKeys[k++]);

                t0 = Fe32_0(x2);
                t1 = Fe32_3(x3);
                x0 ^= t0 + t1 + gSubKeys[k++];
                x0 = RotateRight(x0, 1);
                x1 = RotateLeft(x1, 1) ^ (t0 + 2 * t1 + gSubKeys[k++]);
            }

            UInt32_To_LE((uint)(x2 ^ gSubKeys[OUTPUT_WHITEN]), dst, dstIndex);
            UInt32_To_LE((uint)(x3 ^ gSubKeys[OUTPUT_WHITEN + 1]), dst, dstIndex + 4);
            UInt32_To_LE((uint)(x0 ^ gSubKeys[OUTPUT_WHITEN + 2]), dst, dstIndex + 8);
            UInt32_To_LE((uint)(x1 ^ gSubKeys[OUTPUT_WHITEN + 3]), dst, dstIndex + 12);
        }
        ...
        */
        private void DecryptBlock(
            ...
            byte[] dst,
            int dstIndex) {
            int x2 = (int)LE_To_UInt32(src, srcIndex) ^ gSubKeys[OUTPUT_WHITEN];
            int x3 = (int)LE_To_UInt32(src, srcIndex + 4) ^ gSubKeys[OUTPUT_WHITEN + 1];
            int x0 = (int)LE_To_UInt32(src, srcIndex + 8) ^ gSubKeys[OUTPUT_WHITEN + 2];
            int x1 = (int)LE_To_UInt32(src, srcIndex + 12) ^ gSubKeys[OUTPUT_WHITEN + 3];
            ...
            for (int r = 0; r < ROUNDS; r += 2) {
                ...
                x0 = RotateLeft(x0, 1) ^ (t0 + t1 + gSubKeys[k--]);
                x1 = RotateRight(x1, 1);

                t0 = Fe32_0(x0);
                t1 = Fe32_3(x1);
                x3 ^= t0 + 2 * t1 + gSubKeys[k--];
                x2 = RotateLeft(x2, 1) ^ (t0 + t1 + gSubKeys[k--]);
                x3 = RotateRight(x3, 1);
            }

            UInt32_To_LE((uint)(x0 ^ gSubKeys[INPUT_WHITEN]), dst, dstIndex);
            UInt32_To_LE((uint)(x1 ^ gSubKeys[INPUT_WHITEN + 1]), dst, dstIndex + 4);
            UInt32_To_LE((uint)(x2 ^ gSubKeys[INPUT_WHITEN + 2]), dst, dstIndex + 8);
            UInt32_To_LE((uint)(x3 ^ gSubKeys[INPUT_WHITEN + 3]), dst, dstIndex + 12);
        }
